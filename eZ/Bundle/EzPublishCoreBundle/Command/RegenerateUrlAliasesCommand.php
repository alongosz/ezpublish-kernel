...
EOT
    ...
    private function processLocations(
        ...
        // get published only ContentInfo list
        $contentInfoList = array_filter(
            array_map(
            ...
            function (ContentInfo $contentInfo) {
                return $contentInfo->isPublished();
            }
        );

        $contentList = $this->contentService->loadPublishedContentListByContentInfo($contentInfoList);
        foreach ($locations as $location) {
            try {
                if (!$location->contentInfo->isPublished()) {
                    continue;
                } elseif (!isset($contentList[$location->contentId])) {
